...
def dL_func(ws, examples_pool):
    N = examples_pool['y_hat'].shape[0]
    es = examples_pool['y_hat'] - np.dot(examples_pool['xs'], ws)
    dLs = (-2 / N) * np.dot(np.transpose(examples_pool['xs']), es)
    return dLs
...
def csv_2_dict(filename='train.csv'):
    dict_by_type = {}
...
def feature_scaling_linear(dict_by_type, extend_factor=0.1):
    normal_table = {}
    for key in dict_by_type:
        data = dict_by_type[key]
        _min = np.min(data)
        _max = np.max(data)
        floor = _min * (1 - extend_factor) - _max * extend_factor
        dif = (_max - _min) * (1 + 2*extend_factor)
        if dif == 0.0:
            dif = 1.0
        dict_by_type[key] = (data - floor) / dif
        normal_table[key] = {'min': floor, 'dif': dif}
    return dict_by_type, normal_table




dict_by_type = csv_2_dict('train.csv')
dict_by_type, normal_table = feature_scaling_linear(dict_by_type)


def extract_example(dict_by_type, period):



# examples_number = dict_by_type['PM2.5'].shape[0] * (dict_by_type['PM2.5'].shape[1] - 9)
# feature_number = 9*18
# y_hat = np.zeros(examples_number)
# xs = np.zeros((examples_number, feature_number + 1))
# counter = 0
...
#                        ]
#         counter += 1
# assert counter == examples_number, 'examples_size doesn''t match!'
